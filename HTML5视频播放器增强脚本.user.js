...
/* 当前用到的快捷键 */
const hasUseKey = {
  keyCodeList: [13, 16, 17, 18, 27, 32, 37, 38, 39, 40, 49, 50, 51, 52, 67, 68, 69, 70, 73, 74, 75, 77, 78, 79, 80, 81, 82, 83, 84, 85, 87, 88, 89, 90, 97, 98, 99, 100, 220, 219, 221],
...
const configManager = new ConfigManager({
  ...
  config: {
    ...
    /* 不禁用默认的调速逻辑，则在多个视频切换时，速度很容易被重置，所以该选项默认开启 */
      blockSetPlaybackRate: false,    // Cuipy changed to false
...
const h5Player = {
  ...
  palyerTrigger: function (player, event) {
    ...
    // 按键X或[：减速播放 -0.1
    if (keyCode === 88 || keyCode === 219) {
      t.setPlaybackRateDown();
    }
    // 按键C或]：加速播放 +0.1
    if (keyCode === 67 || keyCode === 221) {
      t.setPlaybackRateUp();
    }
    // 按键Z或\：正常速度播放
    if (keyCode === 90 || keycode === 220) {
      t.resetPlaybackRate();
    }
